...
![gcp](../.vuepress/assets/gcp/250607-1.jpg)

위 그림은 GCP 클라우드 아키텍처 설계 시 고려할 수 있는 리소스들을 정리해둔 장표이다. 시험은 특정 이슈에 대한 최적화 방식을 선택하는 구조로 나오기 때문에 각 리소스들의 역할에 대해 키워드 중심으로 이해하고 있어야 한다.
...
3. 사용할 운영체제를 설정
   - OS 이미지는 퍼블릭 배포된 이미지 또는 커스텀 이미지를 사용할 수 있다.
   - 둘 중 하나를 선택하는 기준은 **기능 요구사항이다.** 커스텀 이미지는 OS 이미지 관리를 온전히 직접 해야한다. 보안 관련하여 엄격한 기준이 있는 회사인 경우 커스텀 이미지를 사용해야 할 필요가 있다.
...
- How to troubleshoot VM not booting up properly (VM 부팅 문제 해결 방법)
  - Serial console 로그 확인으로 부팅 과정 모니터링
  - Cloud Logging에서 시스템 로그 검토
  - 디스크 용량 및 권한 문제 확인
  - 방화벽 규칙이 SSH/RDP 접근을 차단하는지 점검
  - VM 인스턴스의 메타데이터 및 startup script 오류 검사
  - 필요시 rescue mode로 부팅하여 문제 진단
- Custom image vs public image + startup scripts
  - Custom Image 장점:
    - 사전 구성된 소프트웨어와 설정 포함으로 빠른 배포
    - 일관된 환경 보장
    - 부팅 시간 단축
  - Public Image + Startup Scripts 장점:
    - startup scripts는 VM이 시작될 때마다 자동으로 실행되는 shell script 또는 배치 파일이며, 인스턴스 초기 설정, 소프트웨어 설치, 서비스 시작 등을 자동화 해준다.
    - 최신 보안 패치가 적용된 base image 사용
    - 유연한 구성 관리
    - 버전 관리가 용이한 스크립트 기반 설정
    - 다양한 환경에 맞는 동적 구성 가능
- VM price differ between regions (지역별 VM 가격 차이)
  - 각 지역의 인프라 비용과 전력 요금
  - 수요와 공급에 따른 지역별 가격 정책
  - 일반적으로 미국 지역이 가장 저렴
  - 아시아-태평양 지역이 상대적으로 비쌈
  - Sustained Use Discounts와 Committed Use Discounts 활용으로 비용 절감 가능
    - sustained: 1달 중 25% 이상 사용
    - commited: 약정 사용 계약
- Differences between PD images / snapshots / VM images. (PD Images, Snapshots, VM Images 차이점)
  - Persistent Disk (PD) Images:
    - 부팅 가능한 디스크 이미지
    - 새로운 PD 생성 시 사용
    - 지역 간 복제 가능
  - Snapshots:
    ...
    - 전역적으로 사용 가능
  - VM Images:
    - VM 인스턴스 전체의 이미지 (boot disk + metadata)
    - VM 템플릿 생성에 사용
    - 사용자 정의 구성 보존
- PDs are network attach devices as such consume VM bandwidth. (PD는 네트워크로 연결되는 장치이므로 VM의 대역폭을 소비, PD의 네트워크 특성)
  - PD는 네트워크를 통해 연결되는 블록 스토리지
  - VM과 PD 간 데이터 전송은 VM의 네트워크 대역폭을 소비
  - 높은 IOPS나 처리량이 필요한 경우 네트워크 성능 고려 필요
  - Local SSD는 VM에 직접 연결되어 네트워크 대역폭 영향 없음
- VM network performance scales with # of vCPUs. (VM 네트워크 성능과 vCPU 관계)
  - 네트워크 처리량은 vCPU 수에 비례하여 증가
  - 각 vCPU당 약 2Gbps의 네트워크 성능 제공
  - 최대 네트워크 성능은 머신 타입에 따라 제한
  - 고성능 네트워킹이 필요한 경우 충분한 vCPU 할당 필요
  - gVNIC (Google Virtual NIC) 사용 시 더 높은 성능 달성 가능

내부 IP주소는 최소 1개 이상 할당해야 하지만, 외부 IP주소는 선택사항이다.

:::

:::tip Compute Engine Exam tips - Network perspective

1. Network bandwidth limited & dependent on vCPU count (네트워크 대역폭 제한 및 vCPU 의존성)

- 핵심 개념:
  - VM의 네트워크 대역폭은 vCPU 수에 따라 제한됩니다
  - 일반적으로 vCPU당 약 2Gbps씩 증가
  - N2 머신 타입은 최대 ~32Gbps까지 지원
  - Tier1 네트워킹을 사용하면 더 높은 대역폭 확장 가능
- 실무 적용:
  - 고대역폭이 필요한 애플리케이션은 충분한 vCPU를 할당해야 함
  - 네트워크 집약적 워크로드 시 머신 타입 선택이 중요

---

2. You can expect the best network performance for traffic within the same zone, using internal IP addresses (같은 존에서 내부 IP 주소를 사용할 때 최고의 네트워크 성능을 기대할 수 있습니다)

- 최고 성능을 위한 조건:
  - 같은 존(Zone) 내 통신
  - 내부 IP 주소 사용
  - 외부 IP나 다른 존 간 통신은 성능 저하 발생
- 성능 순서 (빠름 → 느림):
  - 같은 존 + 내부 IP
  - 같은 리전, 다른 존 + 내부 IP
  - 다른 리전 + 내부 IP
...
3. Remember about multi-NIC VMs (up to 8) (Multi-NIC VM 지원 - 최대 8개)

- 주요 특징:
  - VM당 최대 8개의 네트워크 인터페이스 지원
  - 각 NIC는 서로 다른 VPC 네트워크에 연결 가능
  - 네트워크 분리 및 보안 강화에 활용
- 사용 사례:
  - DMZ 구성 (프런트엔드/백엔드 네트워크 분리)
  - 네트워크 가상 어플라이언스 (방화벽, 로드밸런서)
  - 하이브리드 클라우드 연결

---

4. Storage is a network resource! => Network bandwidth shared between network AND disk activity (스토리지는 네트워크 리소스입니다! => 네트워크 대역폭이 네트워크와 디스크 활동 간에 공유됩니다)

- 핵심 이해:
  - Persistent Disk는 네트워크를 통해 연결
  - 네트워크 대역폭이 네트워크 트래픽과 디스크 I/O 간에 공유됨
  - 높은 디스크 I/O가 네트워크 성능에 영향을 줄 수 있음
- 실무 고려사항:
  - 총 네트워크 대역폭 = 네트워크 트래픽 + 디스크 I/O\
    - 네트워크 트래픽은 Compute Engine에서 설정하는 대역폭의 60%를 사용하도록 구글에서 고정해두었음.
  - 데이터베이스 서버에서 대용량 백업 수행 시 주의
  - 네트워크 트래픽도 함께 처리하는 웹서버의 경우 충분한 대역폭 필요

:::

- Egress 대역폭이란 VM에서 외부로 데이터를 전송하는 방향의 네트워크 대역폭을 의미합니다.
  - Egress: 송신, 데이터를 밖으로 내보내는 것
  - Ingress: 수신, 데이터를 안으로 받아들이는 것
  - VM → 인터넷, VM → 다른 리전, VM → 외부 서비스로 가는 트래픽이 egress
- GCP에서의 특징
  - vCPU 수에 따라 egress 대역폭도 제한됨 (vCPU당 약 2Gbps)
  - Egress 트래픽에 대해서는 별도 요금 부과
  - 같은 존 내 통신은 egress 요금 없음
  - 리전 간 또는 인터넷으로 나가는 트래픽은 유료
...
상태 기반 어플리케이션과 무상태 어플리케이션은 모두 확장하기는 쉽지만 상태 기반 어플리케이션은 스케일을 축소하기가 어렵다. 일반적으로 statelss한 구조가 선호된다.
